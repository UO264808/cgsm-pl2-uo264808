...
const specialFaceMaterialOn = new THREE.MeshPhongMaterial(
    {
        map: textureLoader.load( "../textures/button_pressed_cube.png", ( loaded ) => { renderer.render( scene, camera ); } ),
        bumpMap: textureLoader.load( "../textures/button_cube_map.png", ( loaded ) => { renderer.render( scene, camera ); } )
    } );// Material for a face
...
const box_2 = new THREE.Mesh( geometry, materials2 );
...
// Define valid movements in the scene
const movements = [
...
document.body.addEventListener( 'keydown', ( event ) => {
    ...
    if ( event.keyCode === spaceKeyCode && intersectedObject ) {
        ...
        if(intersectedObject.name == "box_1"){
            if ( sound_music.isPlaying == true ){
                // Stop the music and update texture
                sound_music.pause();
                // The first face is the one with the button
                box_1.material[ 0 ] =  specialFaceMaterial;
                box_1.material.needsUpdate = true;
            }
            else{
                // Start the music and update texture
                sound_music.play();
                // The first face is the one with the button
                box_1.material[ 0 ] = specialFaceMaterialOn;
                box_1.material.needsUpdate = true;

            }
        }
        else if (intersectedObject.name == "box_2"){
            if ( sound_dog.isPlaying == true ){
                // Stop the music and update texture
                sound_dog.pause();
                // The first face is the one with the button
                box_2.material[ 0 ] = specialFaceMaterial;
                ...
                // Start the music and update texture
                // The first face is the one with the button
                box_2.material[ 0 ] = specialFaceMaterialOn;
                box_2.material.needsUpdate = true;
            }
        }
    }
